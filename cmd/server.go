...
func (s *server) GetProducts(ctx context.Context, in *pb.ProductsRequest) (*pb.ProductsReply, error) {
        ...
        if err != nil {
                fmt.Println(err)
                return &pb.ProductsReply{Message: fmt.Sprintf("Inernal Error: %v", err), Code: 500}, nil
        }
        fmt.Println(string(b))


        return &pb.ProductsReply{Message: string(b), Code: 200}, nil
}
...
func main() {
	...
	grpcSrv := grpc.NewServer(
        ...
        pb.RegisterProductsServer( grpcSrv, &server{})
