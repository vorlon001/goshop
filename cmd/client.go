...
var	targetGRPCAddr = "localhost:18080"
...
func main() {
	...
	path := os.Getenv("goshopgrpc")
	targetGRPCAddr = fmt.Sprintf("%s:18080", path)
	...
	g.Add(func() error {
		for {
			...
			if r, err := cl.GetProducts(metadata.MD(md).ToOutgoing(ctx), &pb.ProductsRequest{}); err != nil {
				return err
			} else {
				fmt.Printf("%#v\n",r)
			}
